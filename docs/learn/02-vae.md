...
**MSE vs. BCE for binary sprites.** Our sprites are essentially binary (pixel is black or white). BCE (Binary Cross-Entropy) is theoretically better for this case — it gives stronger gradients when the prediction is close to 0 or 1, pushing the model toward crisp decisions. MSE treats a 0.1 error at 0.5 the same as at 0.99. We used MSE because it's simpler and "just works," but BCE is a one-line change (`F.binary_cross_entropy` instead of `F.mse_loss`) and likely improves output crispness.
...
**The blurry output problem.** VAEs minimize expected reconstruction error. When a pixel could plausibly be black or white, the optimal MSE prediction is gray (~0.5). This is fundamental to VAEs, not a bug. Palette snapping hides it for evaluation, but the model is genuinely uncertain about those pixels. VQ-VAE (next doc) addresses this by forcing discrete outputs.
